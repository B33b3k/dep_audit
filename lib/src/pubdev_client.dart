  ...
  Future<PackageMetadata?> fetchPackageMetadata(String packageName) async {
    ...
    try {
      final response = await _client
          .get(url, headers: {'Accept': 'application/vnd.pub.v2+json'});

      if (response.statusCode == 200) {
        final data = json.decode(response.body) as Map<String, dynamic>;
        final latestVersion =
            (data['latest'] as Map<String, dynamic>?)?['version'] as String?;
        final publishedString =
            (data['latest'] as Map<String, dynamic>?)?['published']?.toString();

        return PackageMetadata(
          name: packageName,
          latestVersion: latestVersion ?? '0.0.0',
          updatedAt: publishedString != null
              ? DateTime.tryParse(publishedString)
              : null,
